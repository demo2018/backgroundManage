    ...
    return (
      <div className="searchBar">
        ...
        <Form layout="inline">
          <div>
            ...
            <div className="searchWrap">
              <FormItem label="姓名" className="firstLine">
                <Input value={search.realName} onChange={(value) => { this.handleChange('realName', value); }} placeholder="请输入姓名" />
              </FormItem>
              <FormItem label="性别">
                {/* 下拉组件只支持字符串类型，需要手动将数字类型转换为字符串类型 */}
                <Select value={`${search.gender || ''}`} onChange={(value) => { this.handleChange('gender', value); }} placeholder="请选择性别" >
            ...
            <div className="searchWrap">
              <FormItem label="年龄" className="firstLine">
                <Input value={search.age} onChange={(value) => { this.handleChange('age', value); }} placeholder="请输入年龄" />
              </FormItem>
              <FormItem label="出生日期">
          ...
          <FormItem label="来源">
            <Select value={`${search.source || 4}`} onChange={(value) => { this.handleChange('source', value); }} placeholder="请选择来源" >
              {sourceStatus.map(({ label, value }) => (<Option key={`${value}`} value={`${value}`}>{label}</Option>))}
            </Select>
          </FormItem>
          <FormItem label="手机号码">
            <Input value={search.phone} onChange={(value) => { this.handleChange('phone', value); }} placeholder="请输入手机号" />
          </FormItem>
          <FormItem label="机构名称">
            <Input value={search.org} onChange={(value) => { this.handleChange('org', value); }} placeholder="请输入机构名称" />
          </FormItem>
        </Form>
      </div>
    );
