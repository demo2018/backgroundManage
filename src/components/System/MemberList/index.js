...
class MemberList extends React.Component {
  ...
  // 页面渲染
  render() {
    const { getFields, datas, total, roleList, search, loading, addModalVisible, onUpdateState, onAdd, onEdit, onSearch, onReset, selecteRecord, selected = [] } = this.props;
    const { pn, ps } = search;
    const fields = getFields(roleList); // 这样fileds就是动态生成的
    ...
    return (
      <div>
        ...
        <div className="btnGroup">
          <Button type="primary" icon="plus" onClick={() => { onUpdateState({ selecteRecord: {}, addModalVisible: true }); }}>新增成员</Button>
        </div>
        <Table {...tableProps} />
